...
if (isset($_POST['btnTambah'])) {
	...
	$cekQry = mysqli_query($koneksidb, $cekSql) or die("Gagal Query" . mysqli_error($cekQry));
	...
	if (count($pesanError) >= 1) {
		...
		if (mysqli_num_rows($myQry) >= 1) {
			...
			mysqli_query($koneksidb, $tmpSql) or die("Gagal Query tmp : " . mysqli_error(mysqli_query($koneksidb, $tmpSql)));
		}
	}
}
// ============================================================================
...
if (isset($_POST['btnSimpan'])) {
	...
	if (count($pesanError) >= 1) {
		...
		mysqli_query($koneksidb, $mySql) or die("Gagal query" . mysqli_error(mysqli_query($koneksidb, $mySql)));
		...
		while ($tmpData = mysqli_fetch_array($tmpQry)) {
			...
			$itemSql = "INSERT INTO penjualan_item SET 
									...
									jumlah='$dataJumlah'";
			mysqli_query($koneksidb, $tmpSql) or die("Gagal Query " . mysqli_error(mysqli_query($koneksidb, $tmpSql)));
		...
		mysqli_query($hapusSql, $koneksidb) or die("Gagal kosongkan tmp" . mysqli_error(mysqli_query($hapusSql, $koneksidb)));
...
<form action="<?php $_SERVER['PHP_SELF']; ?>" method="post" name="form1" target="_self">
	<table width="800" cellpadding="3" cellspacing="1" class="table-list">
		...
		<tr>
			<td bgcolor="#CCCCCC"><strong>DATA PENGELUARAN </strong></td>
			<td bgcolor="#CCCCCC">&nbsp;</td>
			<td>&nbsp;</td>
		</tr>
		<tr>
			<td width="26%"><strong>No. Pengeluaran </strong></td>
			<td width="2%"><strong>:</strong></td>
			<td width="72%"><input name="txtNomor" value="<?php echo $noTransaksi; ?>" size="23" maxlength="23" readonly="readonly" /></td>
		</tr>
		<tr>
			<td><strong>Tgl. Pengeluaran </strong></td>
			<td><strong>:</strong></td>
		...
		<tr>
			...
			<td><b>:</b></td>
			<td><b>
					<input class="angkaC" name="txtJumlah" size="10" maxlength="4" value="1" onblur="if (value == '') {value = '1'}" onfocus="if (value == '1') {value =''}" />
